...
#include "GPUSkinningSurfaceInput.hlsl"
...
void vert (inout appdata_vert v, out Input o) {
   ...
   o.texcoords.xy = TRANSFORM_TEX(v.uv0, _BaseMap); // Always source from uv0
}
...
void surf (Input IN, inout SurfaceOutputStandard o) {
   ...
   clip (alpha - _Cutoff);
#endif
   o.Albedo = _BaseColor.rgb * tex2D (_BaseMap, texcoords.xy).rgb;
#ifdef _NORMALMAP
...
void final (Input IN, SurfaceOutputStandard o, inout half4 color)
{
   color = OutputForward(color, color.a);
}

void finalSpecular (Input IN, SurfaceOutputStandardSpecular o, inout half4 color)
{
   color = OutputForward(color, color.a);
}
