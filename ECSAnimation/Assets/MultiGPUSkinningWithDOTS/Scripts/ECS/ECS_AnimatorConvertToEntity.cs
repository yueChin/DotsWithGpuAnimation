...
namespace Aoi.ECS
{
    public sealed class ECS_AnimatorConvertToEntity : MonoBehaviour, IConvertGameObjectToEntity
    {
        ...
        void FillEntityWithAnimatorComponent(Entity entity, EntityManager dstManager, ECS_Provider provider)
        {
            ...
            AnimationData animData = default(AnimationData);
            ...
            ECS_SkinnedMatrixAnimator animator = new ECS_SkinnedMatrixAnimator();
        ...
        BlobAssetReference<IntArray> GeneratorIntArr(int[] baseData)
        {
            ...
            {
                ref IntArray root = ref builder.ConstructRoot<IntArray>();
                BlobBuilderArray<int> refArr = builder.Allocate(ref root.ArrayData, baseData.Length);
        ...
        BlobAssetReference<Float3Array> GeneratorFloat3Arr(float3[] baseData)
        {
            using (BlobBuilder builder = new BlobBuilder(Allocator.Temp))
            {
                ref Float3Array root = ref builder.ConstructRoot<Float3Array>();
                BlobBuilderArray<float3> refArr = builder.Allocate(ref root.ArrayData, baseData.Length);
                for (int i = 0; i < baseData.Length; i++)
                {
                    refArr[i] = baseData[i];
                }
        ...
        BlobAssetReference<Float4Array> GeneratorFloat3Arr(float4[] baseData)
        {
            using (BlobBuilder builder = new BlobBuilder(Allocator.Temp))
            {
                ref Float4Array root = ref builder.ConstructRoot<Float4Array>();
                BlobBuilderArray<float4> refArr = builder.Allocate(ref root.ArrayData, baseData.Length);
                for (int i = 0; i < baseData.Length; i++)
                {
                    refArr[i] = baseData[i];
                }
